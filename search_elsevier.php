...
if(!empty($organism_name) && !empty($protein_name)){
	$organism_arr = explode(" ",$organism_name);
	if(count($organism_arr)>1){
		$organism_str = "$organism_arr[0] $organism_arr[1]";
		}
	else{
		$organism_str = $organism_name;
		}
	$protein_name = clean_search_term($protein_name);
	$protein_arr = explode(" ",$protein_name);
	$search_str = "";
	foreach($protein_arr as $protein_term){
		$search_str .= $protein_term." AND " ;
		}
	$search_str .= "\"$organism_str\"";
	shell_exec("python3 search_elsevier.py \"$search_str\" > $uuid.xml 2> $uuid.err");
	if(file_exists("$uuid.xml")){
		$xml_str    = file_get_contents("$uuid.xml");
		$scidir_xml = new SimpleXMLElement($xml_str);
	  $scidir_xml->addChild("query_str","0");
	  $scidir_xml->addChild("organism_name","$organism_name");
	  $scidir_xml->addChild("protein_name","$protein_name");
	  $xml_str = $scidir_xml->asXML();
